...
// --- Non-terminal workspaces are never touched ---

func TestGC_KeepsNonTerminalWorkspaces(t *testing.T) {
	root := t.TempDir()
	cfg := gcConfig(root)
	cfg.GC.TTLMS = 1

	// In Review workspace — not terminal, should never be removed.
	makeDir(t, filepath.Join(root, "UNKNOWN-99"), 200*time.Hour)

	checker := &fakeIssueChecker{} // no terminal issues
	gc := NewGC(cfg, checker, nil, nil)
	result := gc.Collect(context.Background())
	...
	if !dirExists(t, filepath.Join(root, "UNKNOWN-99")) {
		t.Error("non-terminal workspace should not be removed")
	}
}
...
func TestGC_NeverTouchesActiveWorkspaces(t *testing.T) {
	...
	// SYM-5 is not in the terminal set, so it's never touched.
	checker := &fakeIssueChecker{}
	gc := NewGC(cfg, checker, nil, nil)
	result := gc.Collect(context.Background())

	if len(result.RemovedDirs) != 0 || len(result.ArtifactDirs) != 0 {
		t.Errorf("non-terminal workspace should not be touched: removed=%v artifacts=%v",
			result.RemovedDirs, result.ArtifactDirs)
	}
	if !dirExists(t, wsPath) {
...
func TestGC_SkipsNonDirectoryEntries(t *testing.T) {
	...
	if len(result.RemovedDirs) != 0 {
		t.Errorf("RemovedDirs = %v, want empty for non-directory", result.RemovedDirs)
	}
}
...
func TestGC_MixedScenario(t *testing.T) {
	...
	// Non-terminal (active) -> skip
	makeDir(t, filepath.Join(root, "SYM-12"), 10*time.Hour)

	// Non-terminal (unknown/orphan) -> skip (no longer removed)
	...
	// Non-terminal workspace preserved
	if !dirExists(t, filepath.Join(root, "SYM-12")) {
		t.Error("non-terminal workspace SYM-12 should still exist")
	}

	// Previously-orphan workspace now preserved
	if !dirExists(t, filepath.Join(root, "GONE-1")) {
		t.Error("non-terminal workspace GONE-1 should still exist")
	}
}
...
func TestGC_CollectRespectsContextCancellation(t *testing.T) {
	...
	// Create several terminal dirs past TTL.
	for i := 0; i < 5; i++ {
		name := "TERM-" + string(rune('A'+i))
		makeDir(t, filepath.Join(root, name), 2*time.Second)
	}

	checker := &fakeIssueChecker{
		terminal: []internal.Issue{
			{Identifier: "TERM-A", State: "Done"},
			{Identifier: "TERM-B", State: "Done"},
			{Identifier: "TERM-C", State: "Done"},
			{Identifier: "TERM-D", State: "Done"},
			{Identifier: "TERM-E", State: "Done"},
		},
	}
	gc := NewGC(cfg, checker, nil, nil)
